...
export class WebrtcGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async join(client: Socket, payload: PayloadWebrtcDto) {
    try {
      const user = this.webrtcService.join(client, payload);
      this.logger.debug(
        `User ${user.uuid} joined WebRTC channel in conversation ${payload.conversationUuid}`,
      );
      await client.join(payload.conversationUuid);
  ...
  async leave(client: Socket, payload: PayloadWebrtcDto) {
    try {
      const user = this.webrtcService.leave(payload);
      this.logger.debug(
        `User ${user.uuid} left WebRTC channel in conversation ${payload.conversationUuid}`,
      );
      await client.leave(payload.conversationUuid);
  ...
  offer(client: Socket, payload: PayloadWebrtcDto) {
    try {
      const user = this.webrtcService.findUser(
        payload.conversationUuid,
        payload.userUuid,
      );
      this.logger.debug(
        `Offer received from user ${user.uuid} in conversation ${payload.conversationUuid}`,
      );
  ...
  answer(client: Socket, payload: PayloadWebrtcDto) {
    try {
      ...
      this.logger.debug(
        `Answer received from user ${user.uuid} in conversation ${payload.conversationUuid}`,
      );
      client.broadcast
        .to(payload.conversationUuid)
        .emit('answer', payload.data);
    } catch (error: unknown) {
      if (error instanceof Error) {
  ...
  iceCandidate(client: Socket, payload: PayloadWebrtcDto) {
    try {
      ...
      this.logger.debug(
        `Ice-candidate received from user ${user.uuid} in conversation ${payload.conversationUuid}`,
      );
      client.broadcast
        .to(payload.conversationUuid)
        .emit('ice-candidate', payload.data);
    } catch (error: unknown) {
      if (error instanceof Error) {
